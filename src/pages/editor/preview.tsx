...
const PreviewPage = memo((props: PreviewPageProps) => {
  ...
  useEffect(() => {
    ...
    if (!gf) {
      req
        .get<any, any>('/preview/get', { params: { tid } })
        .then(res => {
          const { pageConfig, tpl } = res.data || { pageConfig: {}, tpl: [] };
          console.log('123123', res.data)
          // 设置标题
          document.title = pageConfig.title || '强大的H5编辑神器';
          // 设置数据源
          setPointData(
            tpl.map(item => ({
              ...item,
  ...
  return (
    <>
      ...
      >
        <div ref={refImgDom}>
          <ViewRender pointData={pointData} pageData={pageData} width={vw > 800 ? 375 : vw} />
        </div>
      </div>
